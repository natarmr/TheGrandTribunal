...
app = modal.App("grand-tribunal-inference-v2")
volume = modal.Volume.from_name("grand-tribunal-volume-v2", create_if_missing=True)
...
def strip_think_tags(text: str) -> str:
    """Remove <think>...</think> (or just <think>...) reasoning blocks from Qwen 3.5 output."""
...
inference_image = (
    ...
    .run_commands("hf download Qwen/Qwen3.5-9B")
...
class TribunalModel:
    ...
    async def evaluate_judge(self, topic: str, argument: str):
        ...
        res_text = final_output.outputs[0].text
        print(f"DEBUG evaluate_judge raw response:\n{res_text}")
        return res_text
    ...
    async def generate_character(self, character: str, situation: str):
        ...
        res_text = final_output.outputs[0].text
        print(f"DEBUG generate_character raw response:\n{res_text}")
        return res_text
